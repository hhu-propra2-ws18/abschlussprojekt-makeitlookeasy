...
public class UserController {
   ...
   * 1.If visitor is not logged-in and tries to access
   ...
   * @return 1. Redirect to view "login" (if not logged-in) 2. Display "/accessed/user/profile" view
  ...
  public ModelAndView displayUserProfile(@PathVariable String username, Principal principal,
    ...
    mav.addObject("ppAccount",
        accountHandler.getAccountData(userService.findUserByPrincipal(principal).getUsername()));
    return mav;
  }
  ...
  public ModelAndView editUserProfile(@ModelAttribute @Valid User user,
    ...
    if (user.getUsername().equals(currentPrincipalName)) {
      ...
      LOGGER.warn("Unauthorized access to 'editProfile' for user %s by user %s", user.getUsername(),
